...
*/
  ...
  borrar(login): void {
    this.usuarioApi.borrarUsuario(new Usuario(login, null, null)).subscribe(
      resp => {
        console.log('dnetr');
  ...
  editar(login): void {
    ...
    this.usuarios.forEach((item, index, dat) => {
      if (item.login === login) {
        // this.usuario = item;
        this.usuario = new Usuario(item.login, item.nombre, item.apellido);
      }
    });
  }
  ...
  actualizar(login): void {
    this.usuarioApi.modificarUsuario(this.usuario).subscribe(
      resp => {
        ...
        this.modificarUsuarioArray(new Usuario(resp.body.login, resp.body.nombre, resp.body.apellido1));
      },
      err => {
